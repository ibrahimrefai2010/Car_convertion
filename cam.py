...
interpreter = tf.lite.Interpreter(model_path="94PercentMovQ.tflite")
...
class_names = {0: 'left', 1:'right', 2:'straight', 3:'turn'}
...
try:
    while True:
        ...
        if (prediction == 'left' or prediction == 'turn') and distance >= Threshold and Traffic_prediction == 'Go':
            left()
        elif prediction == 'right' and distance >= Threshold and Traffic_prediction == 'Go':
            right()
        elif prediction == 'straight' and distance >= Threshold and Traffic_prediction == 'Go':
            straight()
        ...
        if photo_count >= 1 and Traffic_prediction == 'Go':
            cv2.imwrite(f'/home/hemo2995/Proggraming/evading/data/{prediction}/B{number}.jpg', raw_image)
            print(f'image saved to path: {prediction} with number: {number}')
            photo_count = 0
            number += 1
        elif Traffic_prediction == 'Go':
            photo_count += 1
